...
static final private int rin = 11; // read integer
    ...
    Parser() {
        String[] S = {"var","constant","procedure"};
        declBeginSys = makeSet(S,3);
        ...
        String[] keywrds = {"begin","call","constant","do","end","if","odd",
                             ...
                             "writestr", "writeln", "readint","for"};
        keywords = makeSet(keywrds,16); // this is the number of keywrds
    }
...
/*    // get next token
    ...
    nextSym = isFactorBeginSymbol(p, sym);
        ...
        case 1 : { // it is an identifier
            ...
            case constant : Main.PL0.gen(lit,0,entry.value); break;
            case variable : Main.PL0.gen(lod,level - entry.level,entry.value); break;
            } sym=getSym(p); break;
        }
...
int processProc (StreamTokenizer p, int level, int baseOffset, Vector followingSymbols) {
    ...
    hashKey = p.sval+"_" + Integer.toString(level);
    if (isIdentifier(p)) {
        symbolTable.put(hashKey, new HashTableValue(procedure,level,baseOffset));
    } else error(3);
    // get semicolon and throw away
    sym=getSym(p);
    lookingFor(p,";",/*"expected semicolon after procedure name",*/4);
    String[] S = {";"};
    block(p, level+1,hashKey, union(programSet,makeSet(S,1)));
    lookingFor(p,";",4);
...
static String operations[] = { "lit ", "opr ", "lod ", "sto ", "cal ", "inc ", "jmp ", "jpc "};
...
static Checkbox store = new Checkbox("store only");
static Checkbox stack = new Checkbox("runtime stack");
static Button load = new Button("Load file");
static Button save = new Button("Save file");
